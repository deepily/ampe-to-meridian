...
| 2026-04-05 | Session 3 | Live Deploy | Root TF composition + 74 resources applied to GCP | Fixes: CMEK service-agent IAM, `user_project_override`, tfstate_location split, `enable_metric_alerts` gate, GCS/KMS region alignment, BQ CMEK drift, Cloud Build SA roles. |
| 2026-04-05 | Session 3 | KFP Pipeline | 8-stage data-prep pipeline on Vertex AI Pipelines (real, not stub) | Cloud Build → Artifact Registry (`meridian/pipeline:v3`). Fixed Dockerfile ENTRYPOINT + `dbdate` parquet round-trip. Full run SUCCEEDED, 37.7 MiB artifacts. |
| 2026-04-05 | Session 3 | Simplification | S.1 io_utils + S.3 logger auto-detect | 8 components refactored; 21 files stripped of `component=` arg. 98/98 tests pass. Plan: `rnd/2026.04.05-live-deploy-and-simplification.md`. |
